...
describe "number", ->
  ...
  it "should count the number of significant digits of a number", ->
  ...
  it "should format a number using toFixed", ->
  ...
  it "should format a bignumber using toFixed", ->
    DECIMAL_PLACES = BigNumber.config().DECIMAL_PLACES
  ...
  it "should format a number using toExponential", ->
  ...
  it "should format a bignumber using toExponential", ->
    DECIMAL_PLACES = BigNumber.config().DECIMAL_PLACES
    BigNumber.config 100
  ...
  it "should convert a number into a bignumber (when possible)", ->
  ...
  it "should convert a bignumber into a number", ->
  ...
  describe "format", ->
    ...
    describe "should apply options", ->
      ...
      it "fixed notation with precision", ->
        options =
          notation: "fixed"
          precision: 2
      ...
      it "auto notation with precision", ->
        assert.equal number.format(1 / 3), "0.3333333333333333"
        assert.equal number.format(1 / 3,
          precision: 3
        ), "0.333"
        assert.equal number.format(1 / 3,
          precision: 4
        ...
        assert.equal number.format(1000.000,
          precision: 5
        ), "1000"
        assert.equal number.format(1000.0010, # rounded off at 5 digits
          precision: 5
        ), "1000"
        assert.equal number.format(1234,
          precision: 3
        ), "1230"
        assert.equal number.format(123.4,
          precision: 6
        ), "123.4"
        assert.equal number.format(0.001234,
          precision: 3
        ), "0.00123"
        assert.equal number.format(1234567,
          precision: 4
        ), "1.235e+6"
        assert.equal number.format(1234567,
          precision: 2
        ), "1.2e+6"
        assert.equal number.format(123e-6,
          precision: 2
        ), "1.2e-4"
        assert.equal number.format(123e-6, # should remove trailing zeros
          precision: 8
        ), "1.23e-4"
        assert.equal number.format(3e+6, # should remove trailing zeros
          precision: 8
        ), "3e+6"
        assert.equal number.format(1234,
          precision: 2
        ...
        assert.equal number.format(2.3,
          precision: 30
        ), "2.3"

      it "auto notation with custom lower and upper bound", ->
        options = exponential:
          lower: 1e-6
          upper: 1e+9
        ...
        assert.equal number.format(Math.pow(2, 53),
          exponential:
            upper: 1e+20
        ), "9007199254740992"

      it "auto notation with custom precision, lower, and upper bound", ->
        options =
          precision: 4
          exponential:
            lower: 1e-6
      ...
      it "should throw an error on unknown notation", ->
        assert.throws ->
          number.format 123,
            notation: "non existing"

    it "should format numbers with precision as second parameter", ->
      assert.equal number.format(1 / 3), "0.3333333333333333"
      assert.equal number.format(1 / 3, 5), "0.33333"
      assert.equal number.format(1 / 3, 3), "0.333"
      assert.equal number.format(2 / 3, 3), "0.667"

    it "should format numbers with a custom formatting function", ->
      asCurrency = (value) ->
        "$" + value.toFixed(2)
    ...
    describe "bignumber", ->
      ...
      it "should format big numbers in exponential notation", ->
        options = notation: "exponential"
      ...
      it.skip "sould format big numbers with custom precision, lower, and upper bound", ->
        oldPrecision = BigNumber.config().DECIMAL_PLACES
        ...
        assert.deepEqual number.format(new BigNumber(5e-40).div(3), options), "0.00000000000000000000000000000000000000016666666666666666666666666666666666666666666666667"
      ...
      it "should format big numbers in fixed notation", ->
        options = notation: "fixed"
