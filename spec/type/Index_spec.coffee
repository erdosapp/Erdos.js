...
describe "Index", ->
  it "should create an Index", ->
    assert.deepEqual new Index([0, 10])._ranges, [
      start: 0
      end: 10
      step: 1
    ]
    assert.deepEqual new Index([0, 10, 2])._ranges, [
      start: 0
      end: 10
      step: 2
    ]
  ...
  it "should calculate the size of an Index", ->
  ...
  it "should calculate the minimum values of an Index", ->
  ...
  it "should test whether an Index contains a scalar", ->
  ...
  it "should test whether an object is an Index", ->
    assert.equal Index.isIndex(new Index()), true
    assert.equal Index.isIndex(erdos.matrix()), false
    assert.equal Index.isIndex(23.4), false
    assert.equal Index.isIndex([]), false
    assert.equal Index.isIndex({}), false
    assert.equal Index.isIndex(new Date()), false

  it "should expand an index into an array", ->
    assert.deepEqual new Index([2, 5]).toArray(), [[2, 3, 4]]
    assert.deepEqual new Index([2, 5], [0, 8, 2]).toArray(), [[2, 3, 4], [0, 2, 4, 6]]
    assert.deepEqual new Index([2, 4], [0, 8, 2], [3, -1, -1]).toArray(), [[2, 3], [0, 2, 4, 6], [3, 2, 1, 0]]

  it "should complain when new operator is missing", ->
    assert.throws ->
      index = Index([2, 5])

  it "should throw an error on non-integer ranges", ->
    assert.throws ->
      new Index([0, 4.5])
