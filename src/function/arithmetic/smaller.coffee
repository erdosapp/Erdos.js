module.exports = (erdos, settings) ->
  ...
  ###
  smaller = (x, y) ->
    throw new error.ArgumentsError("smaller", arguments.length, 2)  unless arguments.length is 2
    ...
    if x instanceof BigNumber
      ...
      return x.lt(y)  if y instanceof BigNumber
    ...
    if y instanceof BigNumber
      ...
      return x.lt(y)  if x instanceof BigNumber
    ...
    return collection.deepMap2(x, y, smaller)  if isCollection(x) or isCollection(y)
    return smaller(+x, y)  if isBoolean(x)
    return smaller(x, +y)  if isBoolean(y)
